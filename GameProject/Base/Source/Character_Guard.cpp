...
Character_Guard::Character_Guard(Vector3 position, Mesh* sprite, PlayerCharacter *player)
: m_active(false)
, player(player)
, m_deadTime(0.f)
...
Character_Guard::Character_Guard(Vector3 position, Mesh* sprite, float velocityx, PlayerCharacter *player)
: m_active(false)
, player(player)
, m_deadTime(0.f)
, Character(position, sprite)
...
void Character_Guard::UpdateVelocity(double dt)
{
	if (m_deadTime == 0.f)
	{
		m_velocity.y += -9.8 * dt;
...
void Character_Guard::UpdatePosition(double dt, const TileMap *tileMap)
{
...
void Character_Guard::UpdateSprite()
{
	if (m_velocity.x < 0)
	{
...
void Character_Guard::Update(double dt, const TileMap *tileMap)
{
